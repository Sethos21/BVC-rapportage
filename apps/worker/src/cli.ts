...
function printGebruik(): never {
  console.error(
    [
      ...
      "      (DELTA BUILD 2026-09-18, uitgebreid t/m Debiteuren/Ouderdomsanalyse (2026-09-23): selecteerbare samengestelde rapportgenerator — combineert de bestaande, ONGEWIJZIGDE productieketens van Pure P&L (pnl-periode), Balans (balans-periode), Huurdersoverzicht (huurdersoverzicht), Kasstroom-managementoverzicht (kasstroom-managementoverzicht), Vastgoed-KPI's (vastgoed-kerncijfers), Controlerapport (controlerapport), RentRoll/huuranalyse (huur-kerncijfers), Servicekosten (servicekosten-positie) en Debiteuren/Ouderdomsanalyse (aansluiting van saldo_huurders op de balanspost Debiteuren) in ÉÉN HTML-document, uitsluitend voor de modules opgegeven in --modules (komma-gescheiden: pnl,balans,huurders,kasstroom,vastgoed_kpi,controles,rentroll,servicekosten,debiteuren — hoofdletterongevoelig). Een niet-geselecteerde module wordt niet uitgevoerd en niet gerenderd. Een geselecteerde maar niet-leverbare module (bv. P&L/Balans/Kasstroom zonder --boekjaar/--periodeTotEnMet, Servicekosten zonder geconfigureerde servicekostenRekeningen, of Debiteuren zonder geconfigureerde debiteurenGrootboekrekeningen óf zonder aansluiting tussen balans en ouderdomsanalyse binnen tolerantie) wordt zichtbaar ONBESCHIKBAAR, blokkeert de andere geselecteerde modules niet. Debiteuren doet bewust GEEN historische openstaand-op-peildatum-reconstructie (bekend, afgesloten BRONGAT — zie packages/reporting/README.md) en toont de Ouderdomsanalyse uitsluitend als haar totaal aansluit op de balans; de balans blijft financieel leidend. Berekent zelf niets — elke sectie is de ongewijzigde Body-renderer van de bestaande module. Nieuwe toekomstige modules sluiten aan via rapportModuleRegister.ts, geen wijziging aan dit commando nodig. HTML naar rapporten/)",
      ...
      "      (TIJDELIJK, alleen-lezen, BRONGATE 2026-09-23 'Historische Ouderdomsanalyse' vervolg op vorderingen-bronkolommen: leest het RUWE vorderingen_met_afboekingen-bronbestand (niet de cache) en toont per geselecteerde vordering ALLE relevante velden SAMEN (rijverband, geen kolomstatistiek) — Bedrijfsnr/Huurdernr/Contractnr/Complexnummer/Unitnummer/Factuurnummer/Datum_Vordering/Vordering_Boekjaar/_Boekperiode/Omschrijving_Vordering/Vordering_Totaalbedrag/Bedrag_afgeboekt/Vordering_openstaand/Vordering_afgehandeld_datum/_jaar/_periode plus de aanwezige Afgeb_bedrag_vsN/Vordering_bedrag_vsN als pure, ongeïnterpreteerde bronwaarden. Selecteert max. ~25 echte rijen in 4 observationele groepen: volledig open (Bedrag_afgeboekt=0, max 5), volledig afgehandeld (Vordering_openstaand=0, bij voorkeur met afgehandeld_datum, max 5), gedeeltelijk afgeboekt (0<|Bedrag_afgeboekt|<|Vordering_Totaalbedrag| en openstaand!=0, max 10), afgehandeld na 30-06-2026 (Datum_Vordering<=30-06-2026 EN Vordering_afgehandeld_datum>30-06-2026, max 5). Geen aging, geen vervaldatum-/betaaldatum-berekening, geen historische reconstructie, geen persoonsgegevens (geen naam/adres/e-mail/telefoon) — alleen JSON op stdout)",
      ...
      "  huurdersoverzicht <administratieId> [--peildatum JJJJ-MM-DD]",
      "      (v1: contract-geankerd huurdersoverzicht — één regel per contract, GEEN rij per rentroll-regel, GEEN kunstmatige unittoewijzing. Hergebruikt bepaalContractGeldigheid uit huur-kerncijfers voor de huur-eligibility (bruto/netto jaarhuur, huurkorting, m², €/m²) — regressie-eis: som over alle contracten reconcilieert exact naar huur-kerncijfers' portefeuillecijfer. Contracteinde/restlooptijd/status komen uit contracten.expiratie_expiratiedatum (NIET afloopdatum, bij 070 vrijwel altijd leeg), met rentroll.contract_expiratiedatum als onafhankelijke controle. Complexomschrijving wordt uitsluitend als aanduiding naast complexnummer getoond, nooit als authoritative complexnaam of voor joins/aggregaties. Servicekostenvoorschot komt uit rentroll.Service_voorschot_jaar (contractueel), NIET uit servicekosten-positie's geboekte voorschotten. Bevat ook Vervallen posten/Openstaande credits (positieve resp. negatieve openstaande detailposten, geclassificeerd op --peildatum — standaard vandaag als niet opgegeven; vervallen = Datum_Vordering + debiteurenbeheer.betaaltermijnDagen (config, default 0) < peildatum). Momentopname, geen boekjaar/periode. Geen renderer/HTML, geen management-rapport-koppeling, alleen JSON op stdout)",
      "  openstaande-posten <administratieId>",
      "      (v1: pure domeinlogica voor openstaande posten — detailbron vorderingen_met_afboekingen (individuele posten, contractattributie, bewezen sleutel Bedrijfsnr+Contractnr+Vordering_Volgnr), control-/ouderdomsbron saldo_huurders (= de cachetabel 'ouderdomsanalyse', huurderniveau officieel saldo + Informant-buckets, NOOIT zelfberekend). Reconciliatie-ernst hangt af van administratie.json's debiteurenbeheer.bankAfletteringDoorOns (true/false/'onbekend', nooit een aanname bij ontbrekende config): true geeft een WAARSCHUWING bij een verschil, false geeft uitsluitend een structurele INFORMATIEF-melding (geen technische foutmelding), onbekend geeft een neutrale WAARSCHUWING. Nog GEEN renderer/HTML, geen koppeling aan Huurdersoverzicht/management-rapport, alleen JSON op stdout)",
      "  contract-verhogingen-diagnose <administratieId>",
      "      (TIJDELIJK, alleen-lezen: onderzoekt bron_gedeeld/contract_verhogingen.xlsx (nog GEEN onderdeel van BRON_TYPES/cache/schema, rechtstreeks gelezen, gefilterd op Bedrijfsnr) op betrouwbaarheid voor historische huurindexaties. Koppeling tegen de bekende contracten uit de cache, chronologische historie per contract incl. VS_01..VS_20-bedragen, en een puur-Decimal (geen JS floating point) vergelijking van de chronologisch laatste regel vóór/op bronPeildatum tegen rentroll's huidige bruto jaarhuur/huurkorting per VS-code. Trekt zelf GEEN conclusie welke VS-code kale huur is of wat Waarde betekent — toont uitsluitend de vergelijkingen. Geen schema/cache-wijziging, alleen JSON op stdout)",
      "  huurdersoverzicht-rapport <administratieId> [--peildatum JJJJ-MM-DD]",
      "      (v1: zelfstandig HTML-rapport van huurdersoverzicht — rendert ongewijzigd het al-bewezen HuurdersoverzichtResultaat, geen nieuwe rekenlogica. --peildatum stuurt de Vervallen posten/Openstaande credits-classificatie (standaard vandaag). Bewust los van management-rapport, om eerst visueel te beoordelen vóór eventuele integratie als sectie of als aparte UI-tab. HTML naar rapporten/)",
...
async function main() {
  ...
  if (command === "contracten-bronkolommen") {
    const [administratieId] = rest;
    if (!administratieId) printGebruik();
  ...
  if (command === "boekingen-bronkolommen") {
    const [administratieId] = rest;
    if (!administratieId) printGebruik();
    const resultaat = genereerBoekingenBronKolommenDiagnose(root, administratieId);
    console.log(JSON.stringify(resultaat, null, 2));
    return;
  }

  if (command === "boekingen-onderhoud-diagnose") {
    const [administratieId] = rest;
    const boekjaarStr = parseFlag(rest, "boekjaar");
    const boekperiodeTotEnMet = parseFlag(rest, "periodeTotEnMet");
    ...
    const resultaat = genereerOnderhoudBoekingenDiagnose(root, administratieId, {
      boekjaar: Number(boekjaarStr),
      boekperiodeVan: parseFlag(rest, "periodeVan"),
      boekperiodeTotEnMet,
  ...
  if (command === "huurdersoverzicht") {
    const [administratieId] = rest;
    if (!administratieId) printGebruik();
    const peildatumStr = parseFlag(rest, "peildatum");
    const peildatum = peildatumStr ? new Date(`${peildatumStr}T00:00:00.000Z`) : new Date();
  ...
  if (command === "openstaande-posten") {
    const [administratieId] = rest;
    if (!administratieId) printGebruik();
    const resultaat = genereerOpenstaandePosten(root, administratieId);
    console.log(JSON.stringify(resultaat, null, 2));
    if (resultaat.controleVereist.some((i) => i.ernst === "KRITIEK")) process.exitCode = 1;
    return;
  }

  if (command === "contract-verhogingen-diagnose") {
    const [administratieId] = rest;
    if (!administratieId) printGebruik();
    const resultaat = genereerContractVerhogingenDiagnose(root, administratieId);
    console.log(JSON.stringify(resultaat, null, 2));
    return;
  }

  if (command === "huurdersoverzicht-rapport") {
    const [administratieId] = rest;
    if (!administratieId) printGebruik();
    const peildatumStr = parseFlag(rest, "peildatum");
    const peildatum = peildatumStr ? new Date(`${peildatumStr}T00:00:00.000Z`) : new Date();
    const resultaat = genereerHuurdersoverzichtRapport(root, administratieId, peildatum);
    console.log(`Huurdersoverzicht geschreven: ${resultaat.pad}`);
