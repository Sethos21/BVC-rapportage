...
function printGebruik(): never {
  console.error(
    [
      ...
      "      (DELTA BUILD 2026-09-18, uitgebreid t/m V3: selecteerbare samengestelde rapportgenerator — combineert de bestaande, ONGEWIJZIGDE productieketens van Pure P&L (pnl-periode), Balans (balans-periode), Huurdersoverzicht (huurdersoverzicht), Kasstroom-managementoverzicht (kasstroom-managementoverzicht), Vastgoed-KPI's (vastgoed-kerncijfers), Controlerapport (controlerapport), RentRoll/huuranalyse (huur-kerncijfers) en Servicekosten (servicekosten-positie) in ÉÉN HTML-document, uitsluitend voor de modules opgegeven in --modules (komma-gescheiden: pnl,balans,huurders,kasstroom,vastgoed_kpi,controles,rentroll,servicekosten — hoofdletterongevoelig). Een niet-geselecteerde module wordt niet uitgevoerd en niet gerenderd. Een geselecteerde maar niet-leverbare module (bv. P&L/Balans/Kasstroom zonder --boekjaar/--periodeTotEnMet, of Servicekosten zonder geconfigureerde servicekostenRekeningen) wordt zichtbaar ONBESCHIKBAAR, blokkeert de andere geselecteerde modules niet. Berekent zelf niets — elke sectie is de ongewijzigde Body-renderer van de bestaande module. Nieuwe toekomstige modules (bv. Liquiditeitsbegroting) sluiten aan via rapportModuleRegister.ts, geen wijziging aan dit commando nodig. HTML naar rapporten/)",
