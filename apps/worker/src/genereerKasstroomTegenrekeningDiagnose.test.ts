...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "bvc-kasstroom-diagnose-"));
  ...
  schrijfXlsxFixture(join(bronGedeeldDir(root), "boekingen.xlsx"), [
    boekingRij({ Boeking_Grootboeknr: "1010", Boeking_Bedrag_Debet: 0, Boeking_Bedrag_Credit: 300 }),
    boekingRij({ Boeking_Volgnr: "000002", Boeking_Grootboeknr: "0840", Boeking_Bedrag_Debet: 300, Boeking_Bedrag_Credit: 0 }),
  ]);
...
describe("genereerKasstroomTegenrekeningDiagnose", () => {
  it("vindt het boekstuk waarin 0840 als tegenrekening van een liquide mutatie voorkomt", () => {
    const resultaat = genereerKasstroomTegenrekeningDiagnose(root, "070_rooisezoom", { boekjaar: 2026, boekperiodeTotEnMet: "06", doelRekening: "0840" });
    expect(resultaat.boekstukken).toHaveLength(1);
    expect(resultaat.boekstukken[0]?.teltNuMeeAlsEigenaarOnttrekking).toBe(true);
    expect(resultaat.totaalBedragMeegeteld.toString()).toBe("300");
  });
});
